...
Alongside with the arrow execution, the generated Netlist is stored inside a graph like data structure called a \hs{CircuitDescriptor}.
\begin{haskell}
...
the other, they are stored inside a tuple. The arrow resides in the first part of the tuple, the graph occupies the second one. A Grid is
the analog to the snap circuit toys from childhood. It is a grid surface where hardware elements can be combined with each other to compose
new structures and analyze them. A \hs{Grid} is not only the surface to combine hardware components, but it also is a recursive surface. So
...
\par 
Another problem with this approach is the compiler. It is not possible for GHC to deduce harder arithmetics in the type context and also is
valid for simpler ones. There are way's to supply GHC with additional information that help to deduce more complicated expressions.


\subsection{Type-Family Solutions}
Another approach is to replace the Horn like clauses by actual calculations. 

\begin{itemize}
  \item what is a type family
  \item type family for addition
  \item arrow class with type families
  \item :+: problem with type families
\end{itemize}
