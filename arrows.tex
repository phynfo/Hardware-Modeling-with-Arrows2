...
The type for e.g. \hs{(***)} is not natural for hardware design. The type of the data going into a circuit is always the same as
the type of the values going out of a circuit. 
...
class VArrow a where
  arr    :: (Nat n, Nat m) => (Vec n b -> Vec m b) -> a (Vec n b) (Vec m b)
  (>>V)  :: a (Vec n b) -> 
  firsts :: 
  (&&&V)  :: 
  (***V)  :: 
