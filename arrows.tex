...
transmitted by bundling multiple boolean ones. Sensor developers tend to digitize pretty early in their design. \cite{MakinwaSmartSensor}

Both the type of an arrow \hs{a b c} itself and the type of the arrows combinators do not match natural for hardware design. The type 
\hs{a b c} is too general for a logic gate: type of the data going into a logic gate should always be the same as the type of the values
going out of this logic gate. Secondly, a set of wires actually is not a tuple and, for instance, the tuple \hs{(a,(b,(c,d)))} represents
the same set of wires as the tuple \hs{(a,((b,c),d))} and the number of possible (different with respect to syntax and type) representations
of a set of wires exponentially explodes with a growing number wires we are trying to model.
...
type of values but are static in their amount of their values. So arrows are exactly opposing the hardwares nature. The question is, can
arrows be transformed to different kind of arrows? And in such a way that they match hardware at more naturally, without loosing the other
beneficial properties for hardware design. 

\par
The obvious solution is to replace the tuple in the arrow structure with a vector. Vectors offer the same behavior as hardware in that they
are static in their data type and dynamic in their value amount. We like to sketch some ways to express such ``Vector-Arrows'' in Haskell
and therefore present \emph{Horn-Clauses}, \emph{Type-Families} and \emph{Dependently Typed Arrows}.


\subsection{Horn-like-clause Solution}
A list can hold an endless amount of elements, so a list matches the hardware with a bunch of wires much better than a tuple. Yet still
there is a problem, a list can always hold more elements than the component hold wires. Therefore it is necessary to bring the count of
wires into the datatype. In a first step there are natural numbers needed inside the data type. With type level natural numbers it is
possible to express how many values a list should hold, respective how many wires a given hardware component holds. 
% TODO, cite richtig platzieren
\cite{kiselyov:strong_collections}

\par
With the next step, the list needs to be restricted to only the exact number of arguments that the type level natural number advises. 


\begin{itemize}
  \item Type Level Natural numbers
